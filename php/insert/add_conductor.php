<?php 
    ...
    require "../log.php";
	if (isset($_POST["nombre"]) && isset($_POST["empresa"]) && isset($_POST["ruta"]) && isset($_POST["departamento"]) && isset($_POST["tipo"]) && isset($_POST["fecha"]) && isset($_POST["unidad"])){
        ...
        $usuario = $_SESSION['user'];
            $sql =$con->query("INSERT INTO conductor (nombre,activo,departamento,id_ruta,empresa,id_usuario,fecha,tipolic,fechalic,id_unidad) VALUES ('$nombre',1,'$departamento',$ruta,'$empresa',$id,'$fecha','$tipo','$fechalic',$unidad);");
            if($ruta == "1"){
                  if($unidad == "1"){

                  }else{
                        $sql =$con->query("UPDATE unidades SET asignada = 1 WHERE id_unidad = $unidad;");
                  }
            }else{
                  if($unidad == "1"){
